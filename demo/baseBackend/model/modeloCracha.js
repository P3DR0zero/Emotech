...
// Alterei para ser possível dar imports no futuro tanto nos tipos de emoção quanto na class.

export const TIPO_EMOCAO = {
  FELIZ: "feliz",
  NEUTRO: "neutro",
  TRISTE: "triste"
};
...
export class RegistroEmocao {
  ...
  constructor(id, emocao, timestamp = new Date()) {
    ...
    //Garante a instância de Date
    this.timestamp = timestamp instanceof Date ? timestamp : new Date(timestamp);
  }
  //Como foi comentado de usar arquivo JSON, vou deixar aqui um método útil para formatar dados para o dashboard (se não precisarem, so comentar o código). A função do JSON tá na classe, então tá tranquilo.
  toJSON() {
    return {
      id: this.id,
